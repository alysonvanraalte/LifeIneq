...
All decompositions covered by this package are additive and follow the same general form:
\begin{align*}
...
weighted sum of the given inequality measure applied to each group. That is, the 
...
of lifespan disparity, $e^\dagger$ (also called _average life lost at death_). Let $N$ be the size of the population
...
`"theil"` (Theil T, specifically, right Christian?), `"edag"`,`"var"`,`"mld"`,`"gini"`,`"mad"`,`"aid"`, and `"H"` are supported. Finally,
...
[TR: maybe also cite Hal's paper here?]
